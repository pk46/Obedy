...
class Kobyla(Restaurant):
    ...
    def __get_only_important_data(self, menu):
        ...
        self.__date = pattern.findall(food[0].text)[0]
    ...
    def _process_data(self, data):
        ...
        for i, food_items in enumerate(menu_data.values()):
            ...
            for item in food_items:
                clean_food = item.replace(self._days[i + 1], "").replace("\t", " ") if i < 4 else (
                    item.replace(self._days[i], "")
                    .replace("\t", " "))
                food.append(clean_food)
            self._menu[self._days[i] + " " + self.__date] = food
